...
def test_report_has_model_and_per_document_levels():
    ...
    rep = score_model(sm, [m for m in metrics if m.owner_model == "orders"], find_collisions(metrics))
    d = rep.to_dict()
    # model level
    ...
    assert all(i["provenance"] == "deterministic" for doc in d["documents"].values() for i in doc["issues"])
...
def test_absent_docs_are_absent_not_zero():
    ...
    assert rep.documents["few_shot"].status == "absent" and rep.documents["few_shot"].score is None
...
def test_document_report_mechanical_score_is_float_or_none():
    ...
    for doc_type, doc in d["documents"].items():
        if doc["status"] == "absent":
            assert doc["mechanical"] is None, f"{doc_type} should have mechanical=None when absent"
        else:
            assert isinstance(doc["mechanical"], float), f"{doc_type} mechanical should be float"
...
def test_issues_have_all_required_fields():
    ...
    bad_sm = NormalizedModel(
        name="orders", source_file="orders.yml", spec="latest",
    ...
    if all_issues:
        required = {"severity", "dimension", "rule", "message", "location", "provenance"}

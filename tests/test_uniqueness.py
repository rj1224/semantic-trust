...
def _nm(name, type_="simple", definition_norm=None, desc="d", owner="team", file_=None):
    ...
    return NormalizedMetric(
        name=name, type=type_, definition_norm=dn,
        description=desc, owner=owner, source_file=file_ or f"{name}.yml"
    )
...
def test_normalize_is_order_independent():
    ...
    a = _nm("rev", definition_norm='{"type":"ratio","type_params":{"denominator":"y","numerator":"x"}}')
    b = _nm("revenue", definition_norm='{"type":"ratio","type_params":{"denominator":"y","numerator":"x"}}')
...
def test_detects_duplicate_formula_under_different_names():
    ...
    ms = [_nm("rev", definition_norm=shared_norm), _nm("revenue", definition_norm=shared_norm)]
...
def test_shared_formula_cross_owner_no_collision():
    ...
    assert formula_cols == [], f"Expected no formula collision across distinct owners, got: {formula_cols}"

...
def _report():
    ...
    ms = load_metrics(FIX)
    return score_model(
        sm, [m for m in ms if m.owner_model == "orders"], find_collisions(ms)
    )
...
def test_judgment_adds_quality_without_touching_trust():
                    ...
                    "issues": [
                        {
                            ...
                            "rule": "description_business_meaning",
                            "message": "restates the formula",
    ...
    assert (
        ...
        out.band,
        dict(out.gates),
    ) == before  # deterministic untouched
    assert out.documents["metrics"].to_dict().get("document_quality") == 62
    assert any(
        i["provenance"] == "llm_judge"
        for i in out.documents["metrics"].to_dict()["issues"]
    )
...
def test_judgment_cannot_flip_a_gate():
    base = _report()
    out = apply_judgment(
        base, {"override_gates": {"uniqueness": False}, "documents": {}}
    )  # malicious payload
...
def test_judgment_unknown_doc_key_ignored():
    ...
    base = _report()
    out = apply_judgment(
        base, {"documents": {"nonexistent_doc": {"quality": 99, "issues": []}}}
    )
    assert out.trust_score == base.trust_score
...
def test_quality_clamped_above_100():
    ...
    base = _report()
    out = apply_judgment(
        base, {"documents": {"metrics": {"quality": 150, "issues": []}}}
    )
...
def test_quality_clamped_below_0():
    ...
    base = _report()
    out = apply_judgment(
        base, {"documents": {"metrics": {"quality": -50, "issues": []}}}
    )
...
def test_quality_non_int_leaves_document_quality_unset():
    ...
    base = _report()
    out = apply_judgment(
        base, {"documents": {"metrics": {"quality": "bad_value", "issues": []}}}
    )
    assert out.documents["metrics"].document_quality is None

...
GOOD_SM = NormalizedModel(
    name="orders", source_file="orders.yml", spec="legacy",
...
GOOD_METRIC = NormalizedMetric(
    name="qcommerce_total_revenue", type="simple",
    definition_norm='{"type":"simple","type_params":{"measure":"order_total"}}',
    description="Sum of all order amounts.",
    owner="data-team", source_file="orders.yml",
)
...
def test_missing_owner_fails_ownership_gate_caps_F():
    m = NormalizedMetric(
        name=GOOD_METRIC.name, type=GOOD_METRIC.type,
        definition_norm=GOOD_METRIC.definition_norm,
        description=GOOD_METRIC.description,
        owner=None, source_file=GOOD_METRIC.source_file,
    )
...
def test_missing_description_fails_completeness_gate():
    m = NormalizedMetric(
        ...
        definition_norm=GOOD_METRIC.definition_norm,
        description="",
        owner=GOOD_METRIC.owner, source_file=GOOD_METRIC.source_file,
    )
    r = score_model(GOOD_SM, [m], [])
...
def test_latest_spec_model_scores_same_as_legacy():
    ...
    sm_latest = NormalizedModel(
        ...
        entities=GOOD_SM.entities, dimensions=GOOD_SM.dimensions,
        measures=GOOD_SM.measures, has_time_dimension=True,
    )

...
def test_skills_use_dbt_parse_gate_not_mf_only():
    for f in (
        ...
        if "mf validate-configs" in txt or "mf compile" in txt:
            ...
            assert re.search(r"legacy", txt, re.I), (
                f"{f} mentions mf without legacy qualification"
            )
...
def test_corpus_no_semantic_type():
    ...
    assert "semantic_type:" not in txt, (
        "qcommerce-corpus.md contains forbidden 'semantic_type:'"
    )
